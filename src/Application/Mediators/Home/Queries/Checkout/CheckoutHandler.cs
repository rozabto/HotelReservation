...
namespace Application.Home.Queries.Checkout
{
    public class CheckoutHandler : IRequestHandler<CheckoutQuery, CheckoutResponse>
    {
        ...
        public Task<CheckoutResponse> Handle(CheckoutQuery request, CancellationToken cancellationToken)
        {
            ...
            var checksum = CalculateChecksum(_paramsForChecksum, _configuration.GetValue<string>("Key:SafeCharge"), ENCODING);
            _params.Add("checksum", checksum);
            var encoded_params = EncodeParameters(_params, ENCODING);
        ...
        private Dictionary<string, string> ConstructParametersMap(string encoding)
        {
            ...
            var timestamp = DateTime.UtcNow.ToString(DATE_FORMAT);
